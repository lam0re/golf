...
function run_code($input, $test) {
	$path = '/var/golf/perl/' . md5($input) . '/';
	$file = 'input.pl';

	mkdir($path);
	file_put_contents($path . $file, $input);

	return exec('docker run -i --rm -m 8M -v ' . escapeshellarg($path) . ':/foo perlgolf /usr/bin/perlgolf ' . escapeshellarg($test));
}
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
 ...
 <body>
  <div class="container">
   <h1>Goal</h1>
   <span>Write a Perl program that takes a parameter as input and outputs a filtered version with alternating upper/lower case letters of the (english) alphabet. Non-letter characters have to be printed but otherwise ignored.</span>
   <h1>Example</h1>
...
<?php
	...
	if ($output === $expected) {
		if (strlen($_POST['input']) < 39) {
			echo "Very good! You know Perl :) ";
		}
